...
def experiment(command_func, thread_max, n_trials):
    """
...
if __name__ == "__main__":
    if len(sys.argv)<3:
        print('Usage: collect_data.py {save_path} {program_name} "{arguments}" [max_threads] [n_trials]')
        exit(1)

    max_threads=6
    n_trials=5

    program_name = sys.argv[2]      # "path/to/your/program"
    arguments = sys.argv[3]         # arguments, use "arg1 arg2" for multiple
    if (len(sys.argv)>4):
        max_threads = int(sys.argv[4])  # maximum number of threads
    if (len(sys.argv)>5):
        n_trials = int(sys.argv[5]) # number of trials
